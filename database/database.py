...
class Database:
    ...
    def add_expense(self, date, source, category, amount, description, entry_type="expense"):
        ...
        if not query.exec_():
            QMessageBox.critical(None, "Database Error", f"Failed to add expense:\n{query.lastError().text()}")
        else:
            self.load_table()
    ...
    def delete_expense(self):
        """Deletes selected expense row."""
        selected_row = self.main_window.table.currentRow()
        if selected_row == -1:
            QMessageBox.warning(None, "No Expense Chosen", "Please select an expense to delete!")
            return
